...
echo.
echo Checking backend server status on port 5001...
echo.

REM Check if port 5001 is in use
netstat -ano | findstr :5001 >nul
if %errorlevel% equ 0 (
    echo Port 5001 is in use - checking if backend is responding...
    echo.
    ...
    if %errorlevel% equ 0 (
        ...
        echo Port 5001 is in use but backend is NOT responding correctly
        echo.
    ...
    echo Port 5001 is NOT in use - backend server is NOT running
    echo.
